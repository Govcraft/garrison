...
- **The daemon has no audit shipping.** It enrolls, authenticates by signed
  assertion, pulls and enforces its policy bundle, and writes back the bundle
  and checksum it is running (which doubles as its only heartbeat: the
  write-back stamps `last_heartbeat` on each refresh). Nothing ships the audit
  trail to the plane yet.
