...
class ChatViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  private html(webview: vscode.Webview): string {
    ...
    return `<!doctype html><html><head><meta charset="UTF-8">
      ...
      </style></head><body><div id="messages" role="log" aria-live="polite" aria-relevant="additions text" aria-busy="false"></div><div id="composer"><textarea id="input" placeholder="Ask Garrison…"></textarea><div class="actions"><button id="cancel" hidden>Cancel</button><button id="send">Send</button></div></div>
      <script nonce="${nonce}">
        ...
        const add=(kind,text)=>{const el=document.createElement('div');el.className='message '+kind;if(kind==='error')el.setAttribute('role','alert');el.textContent=text;messages.append(el);window.scrollTo(0,document.body.scrollHeight);return el};
