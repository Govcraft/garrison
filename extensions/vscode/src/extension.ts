  ...
  private html(webview: vscode.Webview): string {
    ...
    return `<!doctype html><html><head><meta charset="UTF-8">
      ...
      <script nonce="${nonce}">
        ...
        const add=(kind,text)=>{const el=document.createElement('div'),label=document.createElement('span'),content=document.createElement('span');el.className='message '+kind;if(kind==='error')el.setAttribute('role','alert');label.className='sr-only';label.textContent=(speakers[kind]||'Message')+': ';content.textContent=text;el.append(label,content);messages.append(el);window.scrollTo(0,document.body.scrollHeight);return content};
